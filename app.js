...
function handlePlannerGenerateSubmit(event) {
    ...
    const dailyHours = document.getElementById('plan-daily-hours')?.value;
    ...
    if (scheduleBody && scheduleCard) {
        scheduleBody.innerHTML = `
            <div class="p-4 bg-indigo-50/50 dark:bg-indigo-950/20 border border-indigo-100 dark:border-indigo-900 rounded-xl space-y-2 text-xs text-slate-700 dark:text-slate-300">
                <p><strong>🎯 Study Plan Goal:</strong> Prepare for exams by <strong>${new Date(examDate).toLocaleDateString()}</strong></p>
                ...
                <p><strong>📅 Suggested Split:</strong> ${subjects.split(',').map(s => `<strong>${s.trim()}</strong>: ${(dailyHours / subjects.split(',').length).toFixed(1)} hrs/day`).join(' | ')}</p>
            </div>
        `;
